...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="""Create a face recognition dataset with a set of images.
        There should be one image for each person. The filename will be used as
        its name or id (';' are replaced with ':'). Images must contain only one
        face."""
    )
    ap.add_argument(
        "-c",
...
def main(config_path: Path, image_path: Path, dataset_path: Optional[Path],
    ...
    for image_path in pbar:
        ...
        recognition.register_features(dms[0], image_path.stem.replace(";", ":"))
